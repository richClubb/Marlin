...
bool lcd_sd_status1;//represents SD-card status, true means SD is available, false means opposite.
...
{	
    	...
    	if (sd_status != lcd_sd_status1)
	{
		...
		lcd_sd_status1 = sd_status;

	}
			

	if (CardUpdate && lcd_sd_status1 &&  card.cardOK) // represents to update file list
	{
...
{		
	...
	if(ms > next_rts_update_ms && InforShowStatus)
	{
		...
		if ((power_off_type_yes == 0)  && lcd_sd_status1 && (power_off_commands_count > 0)) // print the file before the power is off.
		{
			if(startprogress == 0)
			{
